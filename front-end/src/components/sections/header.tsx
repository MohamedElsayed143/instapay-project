...
export function Header() {
  ...
  const navLinks = [
    { label: 'Home', href: '#v', active: true },
    { label: 'What is InstaPay?', href: '#h' },
    { label: 'How to?', href: '#t' },
  ];
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 w-full bg-[#5B1F7D] text-white h-[80px] shadow-md font-cairo">
      <div className="container mx-auto h-full px-4 md:px-8 flex items-center justify-between max-w-[1200px]">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="hidden lg:block relative">
            ...
            <div
              className={`absolute right-0 top-full mt-2 w-32 bg-white rounded-md shadow-lg py-1 text-gray-800 transform origin-top transition-all duration-200 ease-out ${
                isLangDropdownOpen ? 'scale-100 opacity-100 visible' : 'scale-95 opacity-0 invisible'
              }`}
            >
              <Link href="?lang=en" className="block px-4 py-2 text-sm hover:bg-gray-100 hover:text-[#5B1F7D]">
                English
              </Link>
              <Link href="?lang=ar" className="block px-4 py-2 text-sm hover:bg-gray-100 hover:text-[#5B1F7D]" style={{ direction: 'rtl' }}>
                العربية
              </Link>
            </div>
          </div>
      ...
      >
        ...
        <div className="absolute right-0 top-0 h-full w-[280px] bg-[#5B1F7D] shadow-2xl p-6 flex flex-col">
          ...
          <div className="mt-6 pt-6 border-t border-white/10">
            <p className="text-sm text-gray-300 mb-3 uppercase tracking-wider font-semibold">Language</p>
            <div className="flex flex-col gap-3">
              ...
              <Link 
                ...
                onClick={() => setIsMobileMenuOpen(false)}
              >
                <span className="w-2 h-2 rounded-full bg-transparent border border-white/50"></span>
                العربية
              </Link>
            </div>
          </div>
