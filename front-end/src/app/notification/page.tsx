...
export default function NotificationsPage() {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/notification/get_notifications.php?user_id=${savedUser.id}`
      );
  ...
  const handleResponse = async (
    ...
    try {
      const response = await fetch(
        "${API_BASE_URL}/transaction/respond_to_request.php",
        {
          method: "POST",
  ...
  return (
    <div
      className={`min-h-screen bg-[#F8F9FA] ${isRtl ? "rtl" : ""}`}
      dir={isRtl ? "rtl" : "ltr"}
    >
      {/* Header */}
            ...
            <div className="relative bg-white/20 p-2 rounded-full">
              ...
              {unreadCount > 0 && (
                <span
                  ...
                  } bg-red-500 text-white text-[10px] font-bold w-5 h-5 flex items-center justify-center rounded-full border-2 border-purple-700 animate-pulse`}
                >
                  {unreadCount}
                </span>
              )}
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  >
                    <div
                      className={`relative p-3 rounded-2xl shrink-0 ${details.color}`}
                    >
                      {details.icon}
                      ...
                      {isPending && (
                        <span
                          ...
                          } w-3 h-3 bg-red-500 border-2 border-white rounded-full`}
                        ></span>
                      )}
                    </div>
                    <div className={isRtl ? "text-right" : ""}>
                      <h3
                        className={`font-bold text-gray-800 text-base ${
                          isPending ? "text-purple-900" : ""
                        }`}
                      >
                        {translateText(n.title)}
                      </h3>
                      <p className="text-gray-500 text-sm">
                        {translateText(n.description)}
                      </p>
                    </div>
                  </div>
                  ...
                  >
                    <div
                      className={`font-black text-lg ${details.amountColor} ${
                        isRtl
                          ? "flex flex-row-reverse gap-1 items-baseline justify-end"
                          : ""
                      }`}
                    >
                    ...
                    <div
                      className={`text-[10px] text-gray-400 mt-1 font-bold flex items-center gap-1 ${
                ...
                {n.type === "payment_request" && isPending ? (
                  <div className="flex gap-3 mt-4 pt-4 border-t border-gray-50">
                    <button
                      ...
                      onClick={() =>
                        handleResponse(n.id, n.request_id, "accepted")
                      }
                    ...
                    <button
                      disabled={processingId === n.id}
                      onClick={() =>
                        handleResponse(n.id, n.request_id, "rejected")
                      }
                  ...
                  !isPending && (
                    <div
                      className={`mt-3 flex items-center gap-1 text-[10px] font-bold text-green-600 bg-green-50 w-fit px-3 py-1 rounded-full uppercase ${
