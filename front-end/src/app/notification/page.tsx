...
export default function NotificationsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA]" dir="ltr">
      {/* Header */}
        ...
        <div className="max-w-4xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-4">
            <div className="relative bg-white/20 p-2 rounded-full">
              ...
              {unreadCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] font-bold w-5 h-5 flex items-center justify-center rounded-full border-2 border-purple-700 animate-pulse">
                  {unreadCount}
                </span>
              )}
            </div>
      ...
      <div className="max-w-4xl mx-auto p-4 space-y-4">
        {loading ? (
          <div className="text-center py-20 text-gray-400 font-medium">Loading activity...</div>
        ) : notifications.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-3xl border-2 border-dashed border-gray-200">
            <Bell className="w-12 h-12 mx-auto text-gray-200 mb-4" />
            <p className="text-gray-500 font-medium">Your notification tray is empty</p>
          </div>
        ) : (
          notifications.map((n) => {
              ...
              >
                <div className="flex justify-between items-center">
                  ...
                  <div className="text-right shrink-0">
                    <div className={`font-black text-lg ${details.amountColor}`}>
                      {details.sign} {parseFloat(n.amount).toLocaleString()}
                      <span className="text-xs ml-1 font-medium">EGP</span>
                    </div>
                    <div className="text-[10px] text-gray-400 mt-1 font-bold flex items-center justify-end gap-1">
                      <Clock size={10} />
                      {n.created_at || n.time}
                    </div>
                  </div>
                ...
                {n.type === "payment_request" && isPending ? (
                  <div className="flex gap-3 mt-4 pt-4 border-t border-gray-50">
                    ...
                    >
                      <Check size={18} />
                      {processingId === n.id ? "Processing..." : "Accept & Pay"}
                    </button>
                    <button
                      disabled={processingId === n.id}
                  ...
                  n.type === "payment_request" && !isPending && (
                    <div className="mt-3 flex items-center gap-1 text-[10px] font-bold text-green-600 bg-green-50 w-fit px-3 py-1 rounded-full uppercase">
                      <Check size={12} /> Transaction Completed
                    </div>
                  )
                )}
              </div>
