...
function CollectMoneyContent() {
  ...
  return (
    <div className="min-h-screen w-full flex flex-col bg-[#f8fafc] bg-gradient-to-br from-[#f8fafc] via-[#f1f5f9] to-[#e2e8f0] font-sans relative overflow-hidden" dir={isRtl ? "rtl" : "ltr"}>
      ...
      <nav className="relative z-50 border-b border-gray-200 backdrop-blur-md bg-white/50 px-6 py-4">
        <div className={`max-w-7xl mx-auto flex items-center justify-between ${isRtl ? 'flex-row-reverse' : ''}`}>
          <Link href="/dashboard" className={`flex items-center gap-2 text-gray-500 hover:text-gray-900 transition-colors group ${isRtl ? 'flex-row-reverse' : ''}`}>
            <ArrowLeft className={`w-5 h-5 transition-transform ${isRtl ? 'rotate-180 group-hover:translate-x-1' : 'group-hover:-translate-x-1'}`} />
            <span className="font-bold text-sm">{t('dash.admin').includes('Admin') ? 'Dashboard' : 'لوحة التحكم'}</span>
          </Link>
      ...
      <div className="flex-1 flex items-center justify-center p-6 relative z-10">
        <div className={`w-full max-w-5xl grid lg:grid-cols-2 gap-12 items-center ${isRtl ? 'text-right' : 'text-left'}`}>
          
          {/* Info Side */}
          <div className="space-y-8">
            <h1 className="text-5xl font-black text-gray-900 leading-tight">
              {t('collect.title')} <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-orange-500">{t('collect.instantly')}</span>
            </h1>
            <p className={`text-gray-500 text-lg font-medium leading-relaxed max-w-md ${isRtl ? 'mr-0 ml-auto' : ''}`}>
              {t('collect.desc')}
            </p>
            <div className="space-y-4">
              {[
                ...
                <div key={item.step} className={`flex items-center gap-4 bg-white border border-gray-100 p-4 rounded-2xl shadow-sm hover:shadow-md transition-all ${isRtl ? 'flex-row-reverse' : ''}`}>
          ...
          <div className="bg-white border border-gray-200 p-10 rounded-[3rem] shadow-[0_20px_50px_rgba(0,0,0,0.05)]">
            <div className={`flex items-center gap-3 mb-10 ${isRtl ? 'flex-row-reverse text-right' : 'text-left'}`}>
                <div className="p-3 bg-purple-50 rounded-2xl">
                    <CircleDollarSign className="text-purple-600" size={24} />
                </div>
                <h2 className="text-2xl font-black text-gray-900 tracking-tight">{t('collect.newReq')}</h2>
            </div>
            ...
            <div className={`space-y-8 ${isRtl ? 'text-right' : 'text-left'}`}>
              <div className="relative border-b border-gray-100 group focus-within:border-purple-500 transition-all">
                <label className="text-[10px] uppercase font-black tracking-widest text-gray-400 mb-2 block">{t('collect.payerPhone')}</label>
                <div className={`flex items-center pb-4 ${isRtl ? 'flex-row-reverse' : ''}`}>
                  <Smartphone className={`w-5 h-5 text-gray-300 group-focus-within:text-purple-600 transition-colors ${isRtl ? 'ml-3' : 'mr-3'}`} />
                  <input
                    type="tel"
                    placeholder="01xxxxxxxxx"
                    value={mobileNumber}
                    onChange={(e) => setMobileNumber(e.target.value)}
                    className={`flex-1 bg-transparent outline-none text-gray-900 font-bold placeholder:text-gray-200 ${isRtl ? 'text-right' : ''}`}
                  />
                </div>
              </div>

              <div className="relative border-b border-gray-100 group focus-within:border-orange-500 transition-all">
                <label className="text-[10px] uppercase font-black tracking-widest text-gray-400 mb-2 block">{t('send.amount')}</label>
                <div className={`flex items-center pb-4 ${isRtl ? 'flex-row-reverse' : ''}`}>
                  <input
                    type="number"
                    placeholder="0.00"
                    ...
                    className={`text-4xl bg-transparent outline-none text-gray-900 font-black w-full placeholder:text-gray-100 ${isRtl ? 'text-right' : ''}`}
                  />
              ...
              {showSuccess && (
                <div className="bg-emerald-50 border border-emerald-100 rounded-2xl p-4 flex items-center justify-center gap-3 animate-bounce">
                  ...
                  <span className="text-emerald-600 text-[11px] font-black uppercase tracking-widest">{t('collect.success')}</span>
                </div>
              )}
              ...
              <button
                ...
                disabled={loading}
                className={`w-full bg-gradient-to-r from-purple-600 to-orange-500 text-white font-black py-5 rounded-[1.5rem] hover:shadow-lg hover:shadow-purple-200 hover:scale-[1.01] active:scale-[0.99] transition-all disabled:opacity-50 flex items-center justify-center gap-3 group mt-4 ${isRtl ? 'flex-row-reverse' : ''}`}
              >
                {loading ? (
